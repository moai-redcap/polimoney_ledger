...
class AddElectionDialog extends StatefulWidget {
  const AddElectionDialog({super.key});

  @override
  State<AddElectionDialog> createState() => _AddElectionDialogState();
}

class _AddElectionDialogState extends State<AddElectionDialog> {
  ...
  Future<void> _save() async {
    ...
    if (_electionDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('選挙の投開票日を選択してください。'), backgroundColor: Colors.red));
      return;
    }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('選挙を追加'),
      content: SizedBox(
        width: MediaQuery.of(context).size.width * 0.4,
        child: _isPoliticiansLoading
